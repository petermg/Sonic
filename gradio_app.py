...
def get_video_res(img_path, audio_path, res_video_path, dynamic_scale=1.0, expand_ratio=0.5, min_resolution=512, inference_steps=25):
...
def process_sonic(image, audio, dynamic_scale, expand_ratio, min_resolution, inference_steps):
    ...
    res_video_path = os.path.abspath(res_path+f'{img_md5}_{audio_md5}_{dynamic_scale}_{expand_ratio}_{min_resolution}_{inference_steps}.mp4')
    if os.path.exists(res_video_path):
        return res_video_path
    else:
        get_video_res(image_path, audio_path, res_video_path, dynamic_scale, expand_ratio, min_resolution, inference_steps)
    return res_video_path
...
inputs = [
    ...
    gr.Slider(0.1, 1.0, value=0.5, step=0.1, label="Expand ratio", info="Controls face expansion during preprocessing"),
    gr.Slider(256, 1024, value=512, step=64, label="Minimum resolution", info="Sets minimum resolution for processing"),
    gr.Slider(10, 50, value=25, step=5, label="Inference steps", info="Number of inference steps for video generation"),
]
...
def get_example():
    return [
        ["examples/image/female_diaosu.png", "examples/wav/sing_female_rap_10s.MP3", 1.0, 0.5, 512, 25],
        ["examples/image/hair.png", "examples/wav/sing_female_10s.wav", 1.0, 0.5, 512, 25],
        ["examples/image/anime1.png", "examples/wav/talk_female_english_10s.MP3", 1.0, 0.5, 512, 25],
        ["examples/image/leonnado.jpg", "examples/wav/talk_male_law_10s.wav", 1.0, 0.5, 512, 25],
    ]
...
demo.launch(server_name='127.0.0.1', server_port=8081, share=True, enable_queue=True)
